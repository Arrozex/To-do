...
import { Task, ViewMode, AIGeneratedTask, Expense, ExpenseCategory } from './types';
...
const App: React.FC = () => {
  ...
  const [expenses, setExpenses] = useState<Expense[]>(() => {
    const saved = localStorage.getItem('droidplan_expenses');
    return saved ? JSON.parse(saved) : [];
  });

  const [monthlyLimit, setMonthlyLimit] = useState<number>(() => {
    const saved = localStorage.getItem('droidplan_limit');
  ...
  useEffect(() => {
    localStorage.setItem('droidplan_expenses', JSON.stringify(expenses));
  }, [expenses]);

  useEffect(() => {
    localStorage.setItem('droidplan_limit', String(monthlyLimit));
  }, [monthlyLimit]);
  ...
  const openModal = (task?: Task, initialDate?: Date) => {
    ...
    // If viewing expenses, reset expense form
  ...
  const handleDeadlineChange = (val: string) => {
    setFormDeadline(val);
    // Automatically sync execution end to deadline
    setFormExecEnd(val);
  };
  ...
  const handleSave = () => {
    ...
    // --- Save Expense ---
    if (viewMode === ViewMode.EXPENSES && !editingTaskId) { // Expense mode logic (simple add)
        if (!expAmount || !expDate || !expNote) {
            setErrorMessage("DATA ERROR: MISSING FIELDS");
            return;
        }
  ...
  const acceptAiSuggestion = (suggestion: AIGeneratedTask) => {
    ...
    const newTask: Task = {
      ...
      deadline: endStr, // Default deadline to end time
  ...
  return (
    <div className="flex flex-col h-screen w-full max-w-md mx-auto relative overflow-hidden bg-black/50 text-slate-200 font-sans scanline">
        ...
        <div className="relative z-10 px-6 py-4 flex justify-between items-end border-b border-cyan-900/50 bg-black/60 backdrop-blur-md">
            ...
            <div className="flex flex-col items-end">
                 <div className="text-[10px] text-slate-400 uppercase tracking-widest mb-1">
                    {viewMode === ViewMode.EXPENSES ? 'Credits Log' : 'Mission Log'}
                 </div>
                 <div className="bg-cyan-950/50 border border-cyan-800 text-cyan-300 px-3 py-1 text-xs font-mono">
                    {viewMode === ViewMode.EXPENSES 
                        ? `TOTAL: $${expenses.reduce((a,b) => a + b.amount, 0).toLocaleString()}` 
      ...
      <div className="relative z-10 flex-1 overflow-y-auto no-scrollbar p-4 pb-28">
        ...
        {/* Task List (Only show if NOT in EXPENSES or STATS mode) */}
        {(viewMode === ViewMode.LIST || viewMode === ViewMode.CALENDAR) && (
      ...
      <div className="relative z-10 bg-black/80 border-t border-white/10 p-2 pb-6 backdrop-blur-xl flex justify-around items-center">
        {[
          ...
          { mode: ViewMode.EXPENSES, icon: Wallet, label: 'FINANCE' },
          { mode: ViewMode.STATS, icon: PieChart, label: 'ANALYTICS' }
        ].map(item => (
            <button 
            key={item.mode}
            onClick={() => setViewMode(item.mode)}
            className={`p-2 rounded flex flex-col items-center gap-1 transition-all duration-300 w-16 group relative overflow-hidden`}
            >
          ...
          <div className="holo-card w-full max-w-md p-6 animate-in slide-in-from-bottom duration-300 border-t border-cyan-500/50 sm:border sm:rounded-lg bg-black/90">
            ...
            <div className="flex justify-between items-center mb-6 border-b border-white/10 pb-2">
              <div className="flex items-center gap-2 text-cyan-400">
                  {viewMode === ViewMode.EXPENSES ? <Coins size={18} /> : <Cpu size={18} />}
            ...
            {viewMode === ViewMode.EXPENSES ? (
                <div className="space-y-5">
                    <div>
                        <label className="block text-[10px] font-bold text-cyan-600 font-tech uppercase tracking-widest mb-1">Description (Note)</label>
                        <input 
                            type="text" 
                            value={expNote}
                            onChange={(e) => setExpNote(e.target.value)}
                            placeholder="e.g. Lunch with team" 
                            className="w-full bg-slate-900/50 border border-slate-700 text-white placeholder-slate-600 rounded-none px-4 py-3 focus:outline-none focus:border-cyan-500 font-mono"
                        />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-[10px] font-bold text-cyan-600 font-tech uppercase tracking-widest mb-1">Amount ($)</label>
                            <input 
                                type="number" 
                                value={expAmount}
                                onChange={(e) => setExpAmount(e.target.value)}
                                placeholder="0.00" 
                                className="w-full bg-slate-900/50 border border-slate-700 text-white rounded-none px-4 py-3 focus:outline-none focus:border-cyan-500 font-mono"
                            />
                        </div>
                        <div>
                             <label className="block text-[10px] font-bold text-cyan-600 font-tech uppercase tracking-widest mb-1">Category</label>
                             <select 
                                value={expCategory}
                             ...
                             >
                                ...
                                <option value="ESSENTIAL">ESSENTIAL (必要)</option>
                                <option value="ENTERTAINMENT">FUN (娛樂)</option>
                                <option value="SUPPLIES">SUPPLIES (用品)</option>
                             </select>
                        </div>
                    </div>
                    <div>
                    ...
                    <div className="pt-2">
                        <Button onClick={handleSave} className="w-full py-4 text-sm shadow-[0_0_20px_rgba(6,182,212,0.2)]">
                            CONFIRM TRANSACTION
                        </Button>
                    </div>
                </div>
